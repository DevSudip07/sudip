...
const Hero = () => {
    return (
        <section
            id="hero"
            className="container mx-auto w-full h-fit pt-25 md:pt-0 min-h-screen flex items-center flex-col-reverse md:flex-row px-4 md:px-40 py-10 gap-10 text-white"
        >
            {/* Left Content */}
            <aside className="w-full text-center md:text-left">
                <small className="text-sm font-semibold text-[var(--p-color)]">Hi, myself</small>
                <h1 className="text-4xl md:text-5xl font-bold mt-2">Sudip Das Ghosh.</h1>
            ...
            <aside className="w-full flex justify-center md:justify-end items-center">
                <div className="w-60 h-60 md:w-[450px] md:h-[450px] rounded-full overflow-hidden shadow-lg bg-amber-400 flex justify-center items-center">
                    <img src={heroImage} alt="Hero" className="w-full h-full object-cover object-top-right md:object-center md:object-contain" />
                </div>
            </aside>
        </section>
    );
